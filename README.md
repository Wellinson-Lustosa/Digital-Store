````markdown
# Digital Store - Projeto de E-commerce

Este é um projeto de desenvolvimento web para o curso Geração Tech, focado na construção de uma aplicação de e-commerce completa utilizando React. A aplicação inclui funcionalidades como listagem e visualização de produtos, carrinho de compras e autenticação de usuário.

**Equipe:**

- [Nome Completo do Membro 1]
- [Nome Completo do Membro 2] (Se aplicável)
- [Nome Completo do Membro 3] (Se aplicável)
...
3.  **Inicie a API Falsa (json-server):**
    Abra um **primeiro terminal** na raiz do projeto e execute o seguinte comando para iniciar o servidor de dados na porta 3001.

    ```bash
    npm run server
    ```

    _Este terminal deve permanecer aberto enquanto você usa a aplicação, pois ele atua como o backend._

4.  **Inicie a Aplicação React:**
    Abra um **segundo terminal** (sem fechar o primeiro) e execute o comando para iniciar a aplicação frontend.
    ```bash
    npm start
    ```
    A aplicação estará disponível para acesso no seu navegador em `http://localhost:3000`.
...
- A propriedade `link` deve ser usada para renderizar um link (na cor `primary` com uma fonte de 18px) do lado direito alinhado vertical com o título. O objeto passado para essa propriedade deve seguir o seguinte padrão
  ...
  {
    "text": "Show More",
    "href": "[https://redirect.link](https://redirect.link)"
  }
  ```
  ...
  [
    {
      ...
      "image": "[https://url.imagem/do/produto1.png](https://url.imagem/do/produto1.png)",
      "price": 200,
      "priceDiscount": 149.9
    },
    {
      "name": "Nome do produto 2",
      "image": "[https://url.imagem/do/produto2.png](https://url.imagem/do/produto2.png)",
      "price": 49.9
    }
  ]
  ```
...
- O componente poderá ter as seguintes propriedades:
  ...
  - `images` é a propriedade mais importante para o funcionamento desse componente. Essa propriedade deve receber como valor um array de objetos seguindo esse padrão
    ```json
     [
         { "src": "[http://site.com/path/to/image1.png](http://site.com/path/to/image1.png)" }
         { "src": "[http://site.com/path/to/image2.png](http://site.com/path/to/image2.png)" }
         { "src": "[http://site.com/path/to/image3.png](http://site.com/path/to/image3.png)" }
         { "src": "[http://site.com/path/to/image4.png](http://site.com/path/to/image4.png)" }
         { "src": "[http://site.com/path/to/image5.png](http://site.com/path/to/image5.png)" }
     ]
    ```
...
\<strong\>Introdução\</strong\>
\</summary\>
A página de listagem de produtos vai renderizar filtros e um lista de produtos, essa pagina vai ser o destino do campo de pesquisa e no menu _Produtos_.
